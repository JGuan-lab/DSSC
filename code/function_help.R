...
QC_first <- function(data, full_phenoData){#
    ...
    lapply(list(libSizes = libSizes, mtPercent = mtPercent,
                rbPercent = rbPercent), filterCells) %>%
        unlist() %>%
...
QC_second <- function(data, full_phenoData, leastNum = 50){
    ...
    return(list(data= data, pData= pData,
...
Split_data <- function(data, pData,cell_counts){
    ...
    train <- data[,training]
    test <- data[,testing]
    return(list(train=train, test=test, pData_train=pData_train,
                pData_test=pData_test, training=training, testing=testing))
}
...
Find_markerGene_limma <- function(train, plotmarker = F,
    ...
    colnames(design) <- unlist(lapply(strsplit(colnames(design),"annotation"),
                                      function(x) x[2]))
...
marker.fc <- function(fit2, log2.threshold = 1, output_name = "markers"){
    ...
    topTable_RESULTS = limma::topTable(fit2, coef = 1:ncol(fit2$coefficients),
                                       number = Inf, adjust.method = "BH",
                                       p.value = 0.1, lfc = log2.threshold)
...
#--------Compose T matrix ------------
Generator <- function(sce, phenoData, Num.mixtures = 1000, pool.size = 100,
    ...
    for(y in 1:Num.mixtures){
        ...
        while(!exists("P")){
            ...
            missing.CT = data.frame(CT = missing.CT,
                                    expected = rep(0, length(missing.CT)), stringsAsFactors = FALSE)
...
marker_strategies <- function(marker_distrib, marker_strategy, C){
    ...
    if(marker_strategy == "all"){
        ...
        markers = marker_distrib %>% dplyr::filter(log2FC > 0) %>%
            ...
            group_by(CT) %>%
            dplyr::top_n(ceiling(n()*0.25), wt=log2FC)
        %>% as.data.frame()
        ...
        markers = marker_distrib %>%
            ...
            dplyr::arrange(CT, desc(log2FC)) %>%
            group_by(CT) %>%
            dplyr::top_n(ceiling(n()*0.5), wt=log2FC) %>%
            as.data.frame()
        ...
        markers = marker_distrib %>%
            ...
            dplyr::arrange(CT, desc(log2FC)) %>%
            group_by(CT) %>%
            dplyr::top_n(ceiling(n()*0.75), wt=log2FC) %>%
            as.data.frame()
        ...
        markers = marker_distrib %>%
            ...
            dplyr::arrange(CT, desc(log2FC)) %>%
            group_by(CT) %>%
            dplyr::top_n(floor(n()*-0.5), wt=log2FC) %>%
            as.data.frame()
...
calculate_result <- function(result_c,result_p,
    ...
    RESULT_c_all = RESULT_c_all %>%
        dplyr::summarise(RMSE = sqrt(mean((observed_values-expected_values)^2)) %>%
                             round(.,4),Pearson=cor(observed_values,expected_values) %>% round(.,4))

    RESULT_p_all = combine_2( P,NORM(result_p))
    RESULT_p_all = RESULT_p_all %>%
        dplyr::summarise(RMSE = sqrt(mean((observed_values-expected_values)^2)) %>%
                             round(.,4),Pearson=cor(observed_values,expected_values) %>%
                             round(.,4))

    RESULT_t_all = combine_2( T,result_c%*% result_p)
    RESULT_t_all = RESULT_t_all %>%
        dplyr::summarise(RMSE = sqrt(mean((observed_values-expected_values)^2)) %>%
                             round(.,4),Pearson=cor(observed_values,expected_values) %>%
                             round(.,4))
    result = NULL
    ...
    if(!is.null(lambda1)){
        ...
        colnames(result) = c("lambda1", "lambda2","lambdaC","RMSE_to_P",
                             "Peason_to_P","RMSE_to_C","Peason_to_C",
                             "RMSE_to_T","Peason_to_T","number_iter","seedd",
                             "TerCondition")
    }else{
        colnames(result) = c("RMSE_to_P","Peason_to_P","RMSE_to_C",
                             "Peason_to_C","RMSE_to_T","Peason_to_T")
    }
...
# Num.mixtures = 1000
# pool.size = 100
Combine_2scdata_hard <- function(scData_1,scData_2,leastNum1=50,leastNum2=50,
                                 plotmarker = F,
                                 Num.mixtures = 1000, pool.size = 100,
                                 cutCT=c("unknown"),
                                 norm1 = 'CPM', norm2 = 'CPM',log2.threshold=1){
...
CountAllResults <- function(result,MyMethodName){
    ...
    MatrixCref <- intersect(c("DSSC3","NNLS", "OLS" ,"FARDEEP", "CIBERSORT",
                              "deconRNASeq","RLR" ,"DCQ" ,"elastic_net" ,
                              "ridge","lasso"  ,"EPIC"),MyMethodName)
    ...
    if(length( myMatrixCref ) != 0 ){
        for (i in 2:length(myMatrixCref)) {
            result$all[i,] <- cbind(result[[myMatrixCref[i]]]$result,
                                    matrix(0,1,6-length(result[[myMatrixCref[i]]]$result)))
        }}
    myScCref <- intersect(ScCref,MethodName)
    if(length( myScCref ) != 0 ){
        num = nrow(result$all)
        for (i in (num+1):(num+length(myScCref))) {
            result$all[i,] <- cbind(result[[myScCref[i-num]]]$result,
                                    matrix(0,1,6-length(result[[myScCref[i-num]]]$result)))
        }}
    ...
    if(length( myNoCref ) != 0 ){
        ...
        for (i in (num+1):(num+length(myNoCref)) ) {
            if(myNoCref[i-num] == "DSSC2"){
                ...
                result$all[i,] <- cbind(result[[myNoCref[i-num]]]$result,
                                        matrix(0,1,6-length(result[[myNoCref[i-num]]]$result)))
            }
