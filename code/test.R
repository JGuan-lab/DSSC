...
p1 <- ggplot()+ 
  geom_bar(data=map_mean,mapping=aes(x=factor(group,levels = a),y=mean,fill=group), 
           ...
           width = 0.7,
           show.legend = F)+  
  scale_fill_manual(values = c("#80AFBF","#80AFBF","#80AFBF","#80AFBF","#80AFBF"))+
  # geom_signif(data=plot_data2,mapping=aes(x=group,y=SOD), 
  #             comparisons = list(c("C", "HT"), 
  #                                c("HI", "HT")),
  #             annotation=c("**"), 
  #             map_signif_level=T, 
  #             tip_length=c(0.04,0.04,0.05,0.05), 
  #             y_position = c(4100,3000), 
  #             size=1, # 修改线的粗细
  #             textsize = 10, 
  #             test = "t.test")+ 
