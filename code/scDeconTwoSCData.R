...
saveRDS(ComDate1,"F:/wangchenqi/CDSC/2scDeconvolution/data2sc/Muraro_Segerstolpe.rds")

ComDate2 <-NULL
ComDate2 <- Combine_2scdata_hard( Baron,Muraro,norm1 = 'cpm',
                                  norm2 =  'none',log2.threshold = log(2))
saveRDS(ComDate2,"F:/wangchenqi/CDSC/2scDeconvolution/data2sc_hard/Baron_Muraro.rds")
...
STRING_name = a[4]; STRING_name
ComDate<- readRDS(paste(getwd(),"/data2sc/",STRING_name,".rds",sep=""))
result <- readRDS(paste(getwd(),"/result/result_",STRING_name,".rds",sep=""))
...
#关于C的ct的热图
STRING_name
map = GetCorMatrix(result$CDSC3$dec$c,ComDate$indata$C,matrix = "c");map
library(pheatmap)
...
eoffice::topptx(plot_pheatmp,
                filename = paste("F:/wangchenqi/CDSC/pictures/class2_easy_NO_ct_",STRING_name,".pptx",sep=""))
...
result$CDSC3$lambda1 <- 0e+00
result$CDSC3$lambda2 <- 1e-02
result$CDSC3$lambdaC <- 1
...
result$CDSC3$result
# 
result$CDSC2$lambda1 <- 0e+00
result$CDSC2$lambda2 <- 1e-01
result$CDSC2$lambdaC <- 0
result$CDSC2$dec = CDSC_2(ComDate$indata$T,  dim(ComDate$indata$C)[2],
...
#----------NNLS----------
require(nnls)
result$NNLS$p = do.call(cbind.data.frame,lapply(apply(ComDate$indata$T,2,function(x) 
  nnls::nnls(as.matrix(ComDate$indata$C_ref),x)),   function(y) y$x))
...
result$elastic_net$p = apply(ComDate$indata$T, 2, 
                             function(z) coef(glmnet::glmnet(x = as.matrix(ComDate$indata$C_ref), 
                                                             y = z, 
                                                             alpha = 0.9, 
                                                             standardize = TRUE, 
...
# alpha=0
require(glmnet)
result$ridge$p = apply(ComDate$indata$T, 2, 
                       function(z) coef(glmnet::glmnet(x = as.matrix(ComDate$indata$C_ref), 
                                                       y = z, 
                                                       alpha = 0.9, 
                                                       standardize = TRUE, 
...
result$MuSiC$p = t(music_prop_my(bulk.eset = ComDate$sc$T.eset, 
                                     sc.eset = ComDate$sc$C.eset, 
                                     clusters = 'cellType',
                                     markers = NULL, 
                                     normalize = FALSE, 
                                     samples = 'SubjectName', 
...
result$SCDC$p <- t(SCDC::SCDC_prop(bulk.eset = ComDate$sc$T.eset, 
                                   sc.eset = ComDate$sc$C.eset, 
                                ct.varname = "cellType", 
                                sample = "SubjectName", 
                                ct.sub = unique(as.character(ComDate$sc$phenoDataC$cellType)), 
...
getwd()
path=paste(getwd(),"/DWLS/results_",STRING_name,sep=""); path;dir.exists(path)
...
result$DSA = CellMix::ged(as.matrix(ComDate$indata$T), ML, method = "DSA", log = F)
...
labels <- Row_label(t(result$TOAST$p),t(ComDate$indata$P));labels
rownames(result$TOAST$p) <- labels
colnames(result$TOAST$c) <- labels
result$TOAST$p <- result$TOAST$p[rownames(ComDate$indata$P),]
result$TOAST$c <- result$TOAST$c[,colnames(ComDate$indata$C)]

result$TOAST$result$c = getPearsonRMSE(result$TOAST$c,ComDate$indata$C)
result$TOAST$result$p = getPearsonRMSE(result$TOAST$p,ComDate$indata$P)
result$TOAST$result$t = getPearsonRMSE(result$TOAST$t,ComDate$indata$T)
...
rownames(ComDate$indata$P)
labels <- Row_label(t(result$CellDistinguisher$p),t(ComDate$indata$P))
rownames(result$CellDistinguisher$p) <- labels
colnames(result$CellDistinguisher$c) <- labels
result$CellDistinguisher$p <- result$CellDistinguisher$p[rownames(ComDate$indata$P),]
result$CellDistinguisher$c <- result$CellDistinguisher$c[,colnames(ComDate$indata$C)]

result$CellDistinguisher$result$c = getPearsonRMSE(result$CellDistinguisher$c,ComDate$indata$C)
result$CellDistinguisher$result$p = getPearsonRMSE(result$CellDistinguisher$p,ComDate$indata$P)
result$CellDistinguisher$result$t = getPearsonRMSE(result$CellDistinguisher$t,ComDate$indata$T)
...
lambdaC <- c(0,1,10^1,10^2,10^3,10^4)
...
saveRDS(para_lambda_44_8,
        paste(getwd(),"/paramater/para_lambda_",STRING_name,".rds",sep=""))
...
for(i in 1:length(a)){
  ...
  result[[i]] <- readRDS(paste(getwd(),"/result/result_",STRING_name,".rds",sep=""))
}
MyMethodName <- c("CDSC3","NNLS","OLS","FARDEEP","CIBERSORT" ,
                  "deconRNASeq","RLR","DCQ","elastic_net","ridge","lasso" ,"EPIC",
                  "MuSiC","Bisque","SCDC", "DWLS",
...
for(i in 1:length(a)){
  ...
  if(nrow(result[[i]]$all)!=length(MyMethodName)){
    print("ERROR")
    break;}
  map <- cbind(map,result[[i]]$all$Peason_to_C)
}
...
eoffice::topptx(plot_pheatmp,filename = 
                  "F:/wangchenqi/CDSC/pictures/class2_easy_NO_Pearson_C.pptx")
# 
# # 泛化能力的箱线图--p----
mapBox <- NULL
...
eoffice::topptx(plot_class1,
                filename = "F:/wangchenqi/CDSC/pictures/class2_easy_NO_fanhua_p.pptx")
# 
# # 泛化能力的箱线图----c-----
mapBox <- NULL
...
eoffice::topptx(plot_class1,filename = 
                  "F:/wangchenqi/CDSC/pictures/class2_easy_NO_fanhua_c.pptx")
...
for(i in 1:length(a)){
  STRING_name = a[i]; STRING_name
  ComDate[[i]] <- readRDS(paste(getwd(),"/data2sc/",STRING_name,".rds",sep=""))
  result[[i]] <- readRDS(paste(getwd(),"/result/result_",STRING_name,".rds",sep=""))
}
rownames(result[[i]]$CDSC3$dec$p)

for (i in 1:length(a)) {
  ctlabels <- Row_label(result[[i]]$CDSC3$dec$c,ComDate[[i]]$indata$C_ref,leastnum=3)
  rownames(result[[i]]$CDSC3$dec$p) <- ctlabels
  colnames(result[[i]]$CDSC3$dec$c) <- ctlabels
  ctlabels_ <- rownames(ComDate[[i]]$P)
...
print(p1) # 打印图片
eoffice::topptx(p1,filename = 
                  "F:/wangchenqi/CDSC/pictures/class2_easy_bar_NO_samples.pptx")
...
# ggtitle("I'm a titile") +theme(plot.title = element_text(hjust = 0.5)) #设置标题居中
plot_class2
eoffice::topptx(plot_class2,filename = "F:/wangchenqi/CDSC/pictures/class2_easy_boxplot_samples.pptx")
